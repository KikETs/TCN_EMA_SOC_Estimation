...
def plot_main_heatmap(details: pd.DataFrame, out_png: Path, out_pdf: Path, min_count: int, dpi: int) -> None:
    ...
    fig.savefig(out_pdf)
    Image.open(out_png).convert("RGB").save(out_png.with_suffix(".tif"), dpi=(dpi, dpi))
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--details",
        default="Data/section2_tables/table_s2_raw_vi_bin_details.csv",
        help="Raw V-I bin detail table generated by the Section 2 measurement-structure analysis.",
    )
    parser.add_argument(
        "--out-png",
        default="Figures/figure_2_vi_bin_soc_spread.png",
    )
    parser.add_argument(
        "--out-pdf",
        default="Figures/figure_2_vi_bin_soc_spread.pdf",
    )
