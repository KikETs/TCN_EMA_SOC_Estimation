...
def build_figures(source: Path, fig_dir: Path) -> None:
    ...
    ax.set_xlabel("ΔMAE vs unperturbed (%)")
    ax.set_ylabel("")
    plt.tight_layout()
...
def main() -> int:
    p = argparse.ArgumentParser(description="Build Figure 7 feature-ablation plot using the original paper-artifact plotting logic.")
    p.add_argument("--base-dir", default=".")
    args = p.parse_args()
    base_dir = Path(args.base_dir).resolve()
    source = base_dir / "Data" / "source_metrics"
    fig_dir = base_dir / "Figures"
    missing = [p.name for p in [source / "feature_ablation_reanalysis.csv"] if not p.exists()]
    if missing:
    ...
    bar_by_temp(
        ...
        "metric_value",
        "Feature Ablation MAE",
        "figure_7_feature_ablation",
        fig_dir,
        large_font=True,
        format_temp_labels=True,
    )
    out_png = fig_dir / "figure_7_feature_ablation.png"
    Image.open(out_png).convert("RGB").save(out_png.with_suffix(".tif"), dpi=(600, 600))
    print(f"Wrote {out_png.relative_to(base_dir)}")
    print(f"Wrote {(fig_dir / 'figure_7_feature_ablation.pdf').relative_to(base_dir)}")
    return 0
