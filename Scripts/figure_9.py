...
def make_feature_figures(const: pd.DataFrame, curves: pd.DataFrame, figures_dir: Path) -> None:
    ...
    labels = {
        ...
        "V_corr_raw_ema50": r"$\mathbf{m}^{(50)}(\mathbf{V}_{t}^{\mathbf{corr}})$",
        "V_corr_raw_ema800": r"$\mathbf{m}^{(800)}(\mathbf{V}_{t}^{\mathbf{corr}})$",
        "I_raw": r"$\mathbf{I}_{t}$",
        "I_raw_ema50": r"$\mathbf{m}^{(50)}(\mathbf{I}_{t})$",
        "I_raw_ema200": r"$\mathbf{m}^{(200)}(\mathbf{I}_{t})$",
        "absI_ema50": r"$\mathbf{m}^{(50)}(|\mathbf{I}_{t}|)$",
    ...
    bar_tick_labels = [
        r"$\mathbf{V}_{t}$",
        r"$\mathbf{V}_{t}^{\mathbf{corr}}$",
        r"$\mathbf{m}^{(50)}(\mathbf{V}_{t}^{\mathbf{corr}})$",
        r"$\mathbf{m}^{(800)}(\mathbf{V}_{t}^{\mathbf{corr}})$",
        r"$\mathbf{I}_{t}$",
        r"$\mathbf{m}^{(50)}(\mathbf{I}_{t})$",
        r"$\mathbf{m}^{(50)}(|\mathbf{I}_{t}|)$",
    ]
    ...
    axes[2].set_xticks(np.arange(len(bar_cols)), bar_tick_labels, rotation=45, ha="right")
    axes[2].tick_params(axis="x", labelsize=12.0)
    for ax in axes:
        for label in ax.get_xticklabels() + ax.get_yticklabels():
            label.set_fontweight("bold")
    for panel_label, ax in zip(("(a)", "(b)", "(c)"), axes):
...
def mirror_figure_9_outputs(base_dir: Path, out_dir: Path) -> None:
    figures_dir = base_dir / "Figures"
    figures_dir.mkdir(parents=True, exist_ok=True)
    src_png = out_dir / "figures" / "Figure_6_feature_frequency_behavior.png"
    src_pdf = out_dir / "figures" / "Figure_6_feature_frequency_behavior.pdf"
    out_png = figures_dir / "figure_9_feature_frequency_behavior.png"
    out_pdf = figures_dir / "figure_9_feature_frequency_behavior.pdf"
    out_tif = figures_dir / "figure_9_feature_frequency_behavior.tif"
...
def main() -> int:
    ...
    (out_dir / "frequency_structure_metadata.json").write_text(json.dumps(metadata, indent=2, default=str), encoding="utf-8")
    mirror_figure_9_outputs(base_dir, out_dir)
    print(f"Wrote frequency analysis outputs to {out_dir}")
    print(f"Wrote Figure 9 to {base_dir / 'Figures' / 'figure_9_feature_frequency_behavior.png'}")
    return 0
