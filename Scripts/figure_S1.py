...
def save_operating_space_fig(fig: plt.Figure, figures: Path, stem: str, right: float = 0.995) -> None:
    fig.subplots_adjust(left=0.06, right=right, bottom=0.09, top=0.985, wspace=0.055, hspace=0.08)
    fig.savefig(figures / f"{stem}.png", dpi=600)
    fig.savefig(figures / f"{stem}.pdf")
    plt.close(fig)


def style_operating_space_axis(ax: plt.Axes) -> None:
    ax.tick_params(axis="both", labelsize=13, width=1.15, length=4.5)
...
def plot_operating_space(all_data: pd.DataFrame, figures: Path, profiles: tuple[str, ...], temperatures: tuple[float, ...]) -> None:
    ...
    for r, temp in enumerate(temperatures):
        for c, profile in enumerate(profiles):
            ...
            ax.set_xlim(vlim)
            ax.set_ylim(ilim)
            style_operating_space_axis(ax)
    fig.supxlabel("Voltage (V)", fontsize=18, fontweight="bold", y=0.018)
    fig.supylabel("Current (A)", fontsize=18, fontweight="bold", x=0.01)
    save_operating_space_fig(fig, figures, "fig_s2_operating_space_vi_grid_density")
    ...
    (figures / "Figure_S1_operating_space_vi_grid_density.png").write_bytes((figures / "fig_s2_operating_space_vi_grid_density.png").read_bytes())
    (figures / "Figure_S1_operating_space_vi_grid_density.pdf").write_bytes((figures / "fig_s2_operating_space_vi_grid_density.pdf").read_bytes())
    ...
    fig.supxlabel("Voltage (V)", fontsize=18, fontweight="bold", y=0.018)
    fig.supylabel("Current (A)", fontsize=18, fontweight="bold", x=0.01)
    cax = fig.add_axes([0.935, 0.20, 0.018, 0.62])
    cbar = fig.colorbar(sc, cax=cax, label="SOC fraction")
    cbar.ax.tick_params(labelsize=13, width=1.15)
    cbar.set_label("SOC fraction", fontsize=16, fontweight="bold")
    for label in cbar.ax.get_yticklabels():
        label.set_fontweight("bold")
    save_operating_space_fig(fig, figures, "fig_s2_operating_space_vi_grid_soccolor", right=0.91)
    (figures / "Figure_S1_operating_space_vi_grid_soccolor.png").write_bytes((figures / "fig_s2_operating_space_vi_grid_soccolor.png").read_bytes())
    (figures / "Figure_S1_operating_space_vi_grid_soccolor.pdf").write_bytes((figures / "fig_s2_operating_space_vi_grid_soccolor.pdf").read_bytes())
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data-root", default="Data/processed", help="Root containing raw or processed terminal measurement CSV files.")
    p.add_argument("--out-dir", default="Data/section2_measurement_structure")
...
def main() -> None:
    args = parse_args()
    build_section2_package(args)
    base_dir = Path(__file__).resolve().parents[1]
    out_dir = Path(args.out_dir)
    if not out_dir.is_absolute():
        out_dir = base_dir / out_dir
    src_png = out_dir / "figures" / "Figure_S1_operating_space_vi_grid_density.png"
    src_pdf = out_dir / "figures" / "Figure_S1_operating_space_vi_grid_density.pdf"
    figures_dir = base_dir / "Figures"
    figures_dir.mkdir(parents=True, exist_ok=True)
    out_png = figures_dir / "figure_S1_vi_support_maps.png"
    out_pdf = figures_dir / "figure_S1_vi_support_maps.pdf"
    out_tif = figures_dir / "figure_S1_vi_support_maps.tif"
    shutil.copy2(src_png, out_png)
    shutil.copy2(src_pdf, out_pdf)
    Image.open(src_png).convert("RGB").save(out_tif, dpi=(600, 600))
    print(f"Wrote Figure S1 to {out_png}")
