...
def compute_soc_percent(frame: pd.DataFrame, capacity_ah: float, initial_soc_percent: float) -> pd.Series:
    if "Discharge_Capacity_Ah" in frame and frame["Discharge_Capacity_Ah"].notna().any():
        discharged = pd.to_numeric(frame["Discharge_Capacity_Ah"], errors="coerce").fillna(method="ffill").fillna(0.0)
...
def prepare_dataset(raw_dir: Path, reference_dir: Path, out_dir: Path, initial_soc_percent: float, capacity_ah: float | None) -> None:
    ...
    for frame in frames:
        ...
        frame["SOC_percent"] = compute_soc_percent(frame, capacity, initial_soc_percent)
        frame = add_features(frame, r0)
        frame = frame[REQUIRED_OUTPUT_COLUMNS]
        out_file = out_dir / f"NMC_{int(temp)}C_{profile}.csv"
...
def main() -> int:
    ...
    prepare_dataset(Path(args.raw_dir), Path(args.reference_dir), Path(args.out_dir), args.initial_soc_percent, args.capacity_ah)
    return 0
