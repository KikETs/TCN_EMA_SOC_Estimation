...
def build_figures(source: Path, fig_dir: Path) -> None:
    ...
    ax.set_title("EMA Correlation Diagnostics")
...
def write_caption_index(artifact_root: Path) -> None:
    ...
    captions = {
        ...
        "fig5_representative_fuds_soc_trajectory": "Representative FUDS profile-holdout SOC estimation trajectory of the frozen G4 CEMA-TCN model. The example trajectory is selected as the FUDS test trajectory whose trajectory-level MAE is closest to the overall temperature-mean MAE of the frozen G4 model. The top panel compares predicted and ground-truth SOC, and the bottom panel shows the corresponding absolute error.",
